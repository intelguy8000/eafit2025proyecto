...
interface SalesFuelChartProps {
  data: WeeklyWithFeatures[];
}

export function SalesFuelChart({ data }: SalesFuelChartProps) {
  const chartData = data.map((d) => ({
    date: d.date,
    sales: d.totalSales / 1_000_000,
    fuelPrice: d.avgFuelPrice,
  }));
  ...
  return (
    <ResponsiveContainer width="100%" height={280}>
      <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
        <defs>
          <linearGradient id="salesFuelGradient" x1="0" y1="0" x2="0" y2="1">
